...
public class ImeiUtil {
    public static String format(String did) {
        if (did.matches("\\d{14,15}")) {
            return formatImei(did);
        } else {
            return formatMeid(did);
        }
    }
    ...
    public static String errimei() {
        String[] cs = new String[]{"595", "502", "598", "679"};
        return formatImei(cs[(int) (Math.random() * cs.length)] + ShortCut.makeErrorImei(11));
    }

    public static void main(String[] args) {
        while (true) {
            System.out.println(errimei());
        }
    }
